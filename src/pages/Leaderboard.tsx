...
const Leaderboard = () => {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      try {
        ...
        const {
          data: scores,
          error
        } = await supabase.from('leaderboard_scores').select(`
            *,
            profiles!inner (
              username,
              avatar_url
    ...
    const channel = supabase.channel('leaderboard-changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
          ...
          <Card className="bg-gradient-card border-border">
            ...
            <CardContent>
              {isLoading ? <div className="space-y-4">
                ...
                </div> : leaderboardData.length === 0 ? <div className="text-center py-8">
                  ...
                  {leaderboardData.map(entry => <div key={entry.id} className="flex items-center gap-3 p-3 rounded-lg bg-background/50">
          ...
          <main className="flex-1 p-8 overflow-auto">
            <div className="max-w-6xl mx-auto space-y-6">
              ...
              <Card className="bg-gradient-card border-border">
                ...
                <CardContent>
                  {isLoading ? <div className="space-y-4">
                      {[...Array(5)].map((_, i) => <div key={i} className="flex items-center gap-4 p-4">
                    ...
                    </div> : leaderboardData.length === 0 ? <div className="text-center py-12">
                      ...
                      {leaderboardData.map(entry => <div key={entry.id} className="flex items-center gap-4 p-4 rounded-lg bg-background/50 hover:bg-background/80 transition-colors border border-border">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              ...
                              {entry.rank <= 3 && <Badge variant="outline" className="bg-gradient-primary border-web3-orange">
                                  {entry.rank === 1 ? 'Legend' : entry.rank === 2 ? 'Champion' : 'Elite'}
                                </Badge>}
                            </div>
                            {entry.change !== 0 && <div className="text-sm text-muted-foreground">
